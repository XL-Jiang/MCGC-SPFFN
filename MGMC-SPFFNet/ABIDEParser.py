...
# Selected pipeline
pipeline = 'cpac'
root_folder = 'D:\dataset\code'
data_folder = os.path.join(root_folder, 'ABIDE/ABIDE_pcp')
phenotype = os.path.join(root_folder, 'ABIDE/ABIDE_pcp/Phenotypic_V1_0b_preprocessed1.csv')
...
def fetch_filenames(subject_IDs, file_type):
    ...
    filemapping = {'func_preproc': '_func_preproc.nii.gz','rois_aal': '_rois_aal.1D'}
...
def feature_selection(features, labels, train_ind, fnum):
    ...
    selector = RFE(estimator, n_features_to_select=fnum, step=400, verbose=0)
...
def get_networks(subject_list, kind, atlas_name="aal", variable='connectivity'):
    ...
    for subject in subject_list:
        fl = os.path.join(data_folder , atlas_name , 'cpac' , 'filt_noglobal',subject,
...
def get_static_affinity_adj(features, pd_dict):
    pd_affinity = create_affinity_graph_from_scores(['SITE','SEX', 'AGE_AT_SCAN'], pd_dict)
